...
export function createArticleMdxComponents() {
  ...
  return {
    ...
    h2: ({ children, ...props }: React.ComponentPropsWithoutRef<"h2">) => (
      <h2 id={getUniqueHeadingId(getTextContent(children), headingCounts)} className={`scroll-mt-28 pt-6 text-2xl font-semibold leading-tight tracking-normal ${articleTextColor}`} {...props}>
        {children}
      </h2>
    ),
    h3: ({ children, ...props }: React.ComponentPropsWithoutRef<"h3">) => (
      <h3 id={getUniqueHeadingId(getTextContent(children), headingCounts)} className={`scroll-mt-28 pt-4 text-xl font-semibold leading-tight tracking-normal ${articleTextColor}`} {...props}>
        {children}
      </h3>
    ),
    // 段落、列表、引用、链接和行内代码分别覆盖排版、颜色和交互细节。
    p: (props: React.ComponentPropsWithoutRef<"p">) => <p className={`break-words font-[ui-sans-serif,system-ui,sans-serif] text-[15px] font-normal leading-[1.75] [overflow-wrap:anywhere] [text-indent:2em] ${articleTextColor}`} {...props} />,
    ...
    li: (props: React.ComponentPropsWithoutRef<"li">) => <li className="pl-1 [&>figure]:my-4 [&>p]:my-0 [&>p]:[text-indent:0]" {...props} />,

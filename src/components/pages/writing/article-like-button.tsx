...
export function ArticleLikeButton({ articleSlug, initialLikes }: ArticleLikeButtonProps) {
  ...
  // baseLikes 只在 initialLikes 变化时重新解析，避免每次渲染重复计算。
  ...
  return (
    <div className="relative inline-flex flex-col items-center gap-2">
      ...
      <div role="status" aria-live="polite" className={`pointer-events-none absolute top-[calc(100%+0.55rem)] rounded-full border border-zinc-200/70 bg-white/90 px-3 py-1 text-xs font-medium text-zinc-500 shadow-[0_10px_26px_rgba(24,24,27,0.08)] backdrop-blur transition duration-200 dark:border-white/10 dark:bg-neutral-900/90 dark:text-neutral-300 ${showTip ? "translate-y-0 opacity-100" : "translate-y-1 opacity-0"}`}>
        谢谢喜欢
      </div>
    </div>
  );

...
export function WritingContent({ selectedCategory, selectedPage }: WritingContentProps) {
  ...
  const pagination =
    totalPages > 1 ? (
      <nav className="flex items-center justify-between gap-3 pt-4 text-sm text-zinc-500 dark:text-neutral-400" aria-label="文稿分页">
        {currentPage > 1 ? (
          <Link href={getPageHref(currentPage - 1, activeCategory)} className="rounded-full border border-zinc-200/80 px-4 py-2 transition hover:border-zinc-300 hover:text-zinc-900 dark:border-white/10 dark:hover:border-white/20 dark:hover:text-neutral-100">
            上一页
          </Link>
        ) : (
          <span className="rounded-full border border-zinc-200/60 px-4 py-2 text-zinc-300 dark:border-white/10 dark:text-neutral-700">上一页</span>
        )}
        ...
        {currentPage < totalPages ? (
          ...
          <span className="rounded-full border border-zinc-200/60 px-4 py-2 text-zinc-300 dark:border-white/10 dark:text-neutral-700">下一页</span>
        )}
      </nav>
    ) : null;

...
import { getDateParts, isTimelineType, type TimelineItem } from "./timeline-data";
...
export function TimelineContent({ timelineItems, selectedType }: TimelineContentProps) {
  ...
  const filteredTimelineItems = activeType ? timelineItems.filter((item) => item.type === activeType) : timelineItems;
  // 按年份组织列表数据，供 TimelineList 外层循环使用。
  const yearGroups = groupTimelineItemsByYear(filteredTimelineItems);
  ...
  return (
    <section className="mx-auto max-w-5xl pb-12 pt-28 md:pb-16 md:pt-32">
      ...
      <div className="space-y-10">
        ...
        <TimelineHero activeType={activeType} itemCount={filteredTimelineItems.length} yearCount={yearGroups.length} initialTimeProgressStats={initialTimeProgressStats} />

...
} from "@/db/queries/guestbook-comments.query";
...
export async function createGuestbookCommentAction(input: {
  content: string;
...
export async function deleteGuestbookCommentAction(input: {
  id: number;
}): Promise<CommentActionResult> {
  const session = await auth.api.getSession({
    headers: await headers(),
  ...
  try {
    const deleted = await deleteGuestbookComment({
      id: input.id,
      userId: session.user.id,
    });
